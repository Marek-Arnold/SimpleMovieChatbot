...
class BotController(object):
    ...
    def query(self, question, session_id):
        ...
        if user_message is not None and controller_message is not None:
            self.history.keep(user_message)
            self.history.keep(controller_message)
            return controller_message, None

        message_attachment = Attachment(content_type='text', content=question)
        message_question = Message()
        message_question.session_id = session_id
        message_question.actor = 'user'
        message_question.attachments = [message_attachment]

        self.history.keep(message_question)

        print('\nLast session context: ')
        print(self.history.get_last_context(session_id))
        print('Query message:')
        print(self.history.query_message(session_id))
        print()



        # TODO: routing
        message_answer = self.bot.query(message_question, session_id, self.history.query_message(session_id))
        ...
        if message_answer.terminated:
            ...
            optional.actor = 'controller'
            optional.attachments = [Attachment(content_type='feedback',
    ...
    def handle_feedback(self, question, session_id):
        ...
        controller_message.session_id = session_id
        controller_message.actor = 'controller'
        controller_message.attachments = [Attachment(content_type='text', content='Thank you for the feedback')]

        user_message = Message()
        user_message.session_id = session_id
        user_message.actor = 'user'
        user_message.attachments = [Attachment(content_type='feedback', content={'percentage':   percentage, 'expectedType': expected_type})]
